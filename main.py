...
def id_input(): #func for now only, change in the future
    """
...
def emotional_analyzer(face_roi): 
    """
...
def main():
    id = id_input() #gets the original ID, and inputs here
    count = 0 #counter
    while True: #Main Loop
         try:     
            ret, frame = video.read() #when camera empty, _src.empty() in func
            ...
            for (x, y, w, h) in faces: #tuple, takes tuple data and uses it within rectangle
                ...
                emotion_dominant, confidence = emotional_analyzer(face_roi) #takes in ONLY face_roi, returns variables and determines theresholds
